...
class BattleGroupContainer(val parent: MainContext) {
    ...
    private fun addBattleGroup(name: String, battleGroupData: BattleGroupData) {
        if (!battleGroupContainer.containsKey(name)) {
            ...
            parent.lm.messenger.log("INFO", "Battle group container", "Create $name already exist", COLOR_RED)
    }
    ...
    fun deleteBattleGroup(name: String) {
        if (battleGroupContainer.containsKey(name)) {
            ...
            parent.lm.messenger.log("INFO", "Battle group container", "Delete $name not exist", COLOR_RED)
    }
    ...
    fun runInStartOfTick() {
        for (bg in battleGroupContainer.values) {
            try {
                ...
                parent.lm.messenger.log("ERROR", "Battle group runInStartOfTick", bg.name, COLOR_RED)
            }
        }
    }
    ...
    fun runNotEveryTick() {
        for (bg in battleGroupContainer.values) {
            try {
                ...
                parent.lm.messenger.log("ERROR", "Battle group runNotEveryTick", bg.name, COLOR_RED)
            }
        }
    }
    ...
    fun runInEndOfTick() {
        for (bg in battleGroupContainer.values) {
            try {
                ...
                parent.lm.messenger.log("ERROR", "Battle group runInEndOfTick", bg.name, COLOR_RED)
            }
        }
    }
